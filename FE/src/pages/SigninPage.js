...
const SignupPage = () => {
  ...
  useEffect(() => {
    if (!dataSend.email || !dataSend.pass) {
      // Hiển thị lỗi
      return;
    }
    if (dataSend) {
      const { email, pass } = dataSend;
    ...
    <LayoutAuthentication heading="SignIn">
      <div className="mr-[20px]">
        <div className="text-center lg:text-[14px] font-medium text-[12px] leading-[18px] lg:leading-[22px]">
          <span className="text-text3">Don't have an account? </span>
      ...
      >
        ...
        <div className="text-[14px] text-primary mt-[15px] cursor-pointer">
          Forgot password
        </div>
        <button
          type="submit"
          className="bg-primary w-[287px] h-[52px] lg:w-[436px] lg:h-[52px] text-white rounded-[10px] leading-[26px] text-[16px] text-[600] mt-[15px]"
        >
          Sign In
        </button>
      </form>
